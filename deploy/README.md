...
两条都失败才向页面报错。三个用到模型的页面共用 `loadBudget`：`MODEL_LOAD_TIMEOUT_MS`
（60 秒）是允许毫无动静的时间，three.js 库加载完成、收到响应头、每收到一段数据都会
重新计时，所以慢但仍在流动的线路不会被掐断（大陆直连 Vercel 实测首字节 20–40 秒、
2.4 MB 约 30 秒），只有 60 秒完全没有进展才超时，学生会看到提示和重试按钮，不再是
转圈后无声消失。成功线路与动画共用 sessionStorage 记忆
