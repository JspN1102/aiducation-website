...
def build_media_config(manifest):
    ...
    image or model is omitted individually; no broad media directory is
    redirected. Animations are the exception: the same MP4 is deployed on the
    page's own origin and published on COS (``videos``), and the browser keeps
    whichever route answers, because Hong Kong networks often lose packets to
    Guangzhou COS while mainland networks are slow to reach Vercel.
    """
    assets = validated_assets(manifest)
    routed = [asset for asset in assets if asset['contentType'] != 'video/mp4']
    ...
    return {
        ...
        'videos': [{'source': asset['source'], 'destination': asset['destination'],
                    'bytes': asset['bytes']} for asset in videos],
    }
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Generate public COS image URLs and Nginx routes.')
