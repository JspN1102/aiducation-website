...
精确映射的图片和模型从 Vercel 发布包中排除，避免重复占用 Deployment Storage；
六段动画例外：同一 MP4 随发布包一起部署，并保留 COS 副本作为第二条线路，
浏览器出错或停滞时自动切换（见 `README.md` 的「COS 教学媒体」）。
代价是每次部署多约 155 MB 静态文件，学生看动画时的 Vercel 流量计入该项目配额。
