...
`media-manifest.json` 记录六个视频和七个模型对象（包含田地原版及优化版）。打包前核对原文件摘要和大小，再生成
...
TTS在每个运行实例另设60秒短期内存缓存，同音频并发读取合并；最多保留128项和16MiB
音频，超过则淘汰内存副本，持久文件保留。完整WAV校验、签名授权和按存储凭据隔离均
保留，缓存上限不代表整个进程或全部并发请求的内存上限。
...
广州每日04:00后将这份既有导出写入PostgreSQL备用库，导入前后保留私密快照；较新的
PG记录不覆盖，冲突整批失败供复核。每日04:45后在隔离集群完整恢复一次最新备份。
这是约一天恢复点的备用副本，不是实时复制，最终切回仍需导出并核对晚到记录。
