...
document.querySelector('#play').onclick=()=>{const source=window.library.source('1-0');window.audio=new Audio(typeof source==='string'?source:URL.createObjectURL(source));window.audio.onended=()=>window.finished=true;window.audio.play().catch(error=>window.playError=error.message);};
...
(async()=>{
 ...
 const browser=await chromium.launch({channel:'msedge',headless:true}),context=await browser.newContext({serviceWorkers:'block'}),page=await context.newPage(),rows=new Map(),requests=[],errors=[],checks=[];
 ...
 await context.route('**/*',async route=>{
  ...
  if(url.pathname==='/api/school-recordings/'){
   ...
   if(route.request().method()==='POST'){const item=route.request().postDataJSON(),key=item.actorId+':'+(item.learningEpoch||'student');rows.set(key,{...item});delete rows.get(key).audio;return route.fulfill({contentType:'application/json',body:JSON.stringify({ok:true,userId:item.actorId,recording:rows.get(key)})});}
 ...
 try{
  ...
  await page.locator('#play').click();await page.waitForFunction(()=>window.finished);check('restored pending audio actually plays',await page.evaluate(()=>!window.playError));
  ...
  const evidence=process.env.RECORDING_EVIDENCE_DIR;if(evidence){fs.mkdirSync(evidence,{recursive:true});fs.writeFileSync(path.join(evidence,'results.json'),JSON.stringify({checks,errors,syntheticOnly:true,paidProviderCalls:0},null,2));}
  console.log(JSON.stringify({ok:true,checks:checks.length,errors,syntheticOnly:true,paidProviderCalls:0}));
