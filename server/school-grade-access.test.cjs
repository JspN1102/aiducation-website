...
function dependencies(person,extra={}){return {'./_lib/school-auth.cjs':{...auth,enabled:()=>true,requireActor:async()=>person,getStore:()=>({get:async()=>null}),...extra},'./_lib/student-store.js':{...studentStore,mode:()=>false},'@vercel/blob':{get:async()=>assert.fail('unexpected Blob read')}};}
