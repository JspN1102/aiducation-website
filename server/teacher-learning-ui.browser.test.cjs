...
async function welcomeChecks(){
 for(const [width,height] of [[320,740],[360,780],[390,844],[768,1024],[1024,768],[1440,900]]){const env=await setup({width,height});try{
  ...
  const beforeGuide=await page.locator('#poem-grid').boundingBox();
  ...
  check(width+'px click shows one short line without shifting the books or requesting speech',state.tts.length===0&&await page.locator('#library-shishi-guide').evaluate((el,before)=>{const b=el.getBoundingClientRect(),mascot=document.querySelector('.library-shishi').getBoundingClientRect(),grid=document.querySelector('#poem-grid').getBoundingClientRect(),p=el.querySelector('p'),r=document.createRange();r.selectNodeContents(p);return !el.hidden&&!document.querySelector(':modal')&&b.left>=0&&b.right<=innerWidth&&b.top>=0&&b.bottom<=innerHeight&&Math.abs(grid.top-before.y)<1&&r.getClientRects().length===1&&r.getBoundingClientRect().right<=b.right-35&&Math.min(Math.abs(b.left-mascot.right),Math.abs(b.top-mascot.bottom))<=14&&document.querySelector('.library-shishi').getAttribute('aria-expanded')==='true';},beforeGuide));
  check(width+'px all covers remain uncropped with comfortable space below',await page.locator('.library').evaluate(el=>[...el.querySelectorAll('.poem-art')].every(art=>{const b=art.getBoundingClientRect();return Math.abs(b.width/b.height-16/9)<.02&&getComputedStyle(art.querySelector('img')).objectFit==='contain';})&&parseFloat(getComputedStyle(el).paddingBottom)>=32));
