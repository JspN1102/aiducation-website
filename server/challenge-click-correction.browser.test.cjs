 ...
 try{for(const engine of ['chromium','webkit']){
  ...
  try{
   ...
   }
   {
    ...
    check(engine+' next handler rejects direct bypass',await page.evaluate(()=>saved.cursor===2&&submissions.length===1&&saved.answers[2].status==='incorrect'));
    ...
    await page.locator('[data-cw=practise]').click();await page.evaluate(()=>candidates=['口']);await draw(page);await page.locator('[data-cw=submit]').click();
    ...
    check(engine+' correction recognition has review context and cannot replace first result',await page.evaluate(()=>recognitionCalls.every(call=>call.context.context.mode==='review')&&saved.answers[2].status==='incorrect'&&submissions.length===0&&saved.writingCorrections[item.id].status==='corrected'));
    ...
    check(engine+' newly finished correction triggers one progress upload without another assessment',await page.evaluate(()=>correctionSaves.length===1&&correctionSaves[0].status==='corrected'&&submissions.length===0));
