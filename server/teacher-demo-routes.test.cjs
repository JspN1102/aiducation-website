...
test('demo export is a real teacher-only XLSX with explicit simulated title/filename and scoped roster',async()=>withAuth(async()=>{
 ...
 const wb=new ExcelJS.Workbook();await wb.xlsx.load(res.body);assert.match(String(wb.getWorksheet('概覽').getCell('B2').value),/模擬/);assert.equal(wb.getWorksheet('學生明細').rowCount,26);assert.match(String(wb.getWorksheet('學生明細').getCell('D2').value),/示範學生/);
}));
