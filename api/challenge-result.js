...
module.exports = async function handler(req,res){
  ...
  try{
    ...
    let correct=null,status=body.status==='skipped'?'skipped':'completed';
    if(body.status!=='skipped'&&item.type==='sound'){
      ...
      if(!Array.isArray(placements)||placements.length!==slots.length||new Set(placements.map(p=>p.slotId)).size!==slots.length||placements.some(p=>!slots.includes(p.slotId)||typeof p.choiceId!=='string'))return res.status(400).json({error:'Invalid placements'});
    ...
    const saved=await research.recordVerifiedOutcome(req,{provider:'aiducation',model:'curriculum-answer-key',operation:'challenge',providerVersion:'challenge-v'+CHALLENGE_VERSION+'-20260919d',
      result:{status,score:correct===null?null:correct?100:0,correct}});
