  ...
  if(payload.reportStyle==='narrative-teaching-review'){
    ...
    for(const {value,path}of textFields(analysis)){
      ...
      for(const sentence of value.split(/[。！？!?\n]/u)){
        ...
        // Merely planning "分組練習默寫與聽辨" does not claim an observed
        // intersection. Inspect actual paired results, not activity order.
        if(/同一批|交集|重[疊叠]|兩項|两项|低[於于]\s*60/u.test(sentence)){
