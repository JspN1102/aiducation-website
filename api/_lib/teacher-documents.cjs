...
function rangeLabel(dataset){const f=dataset.filters;return `${f.grade?f.grade+' 年級':'全校'}${f.cls?' '+f.cls+' 班':''}${f.student?' · 個別學生':''} · ${f.from} 至 ${f.to}（UTC 收件日期） · ${f.activity?ACTIVITY[f.activity]||f.activity:'全部活動'} · ${f.attempt==='first'?'首次':'最近'}紀錄`;}
