...
module.exports = async function handler(req, res) {
  ...
  // Build the prompt with error analysis
  ...
  const prompt = `你是普通話語音教師，為香港小二學生寫朗讀診斷報告。報告的讀者是學生本人（7-8歲小朋友）。
學生朗讀唐詩《${poem || '贈汪倫'}》，AI評測數據如下：
...
- 用帶聲調拼音標注正確讀法（如 chéng、zhōu）
- 語氣像幼兒園老師對小朋友說話，特別親切溫暖有趣
- 適合7-8歲孩子閱讀，用簡單的詞語

用繁體中文寫診斷報告，結構如下：
1. 整體評價（先大力表揚，用可愛的比喻，再溫柔指出需要加油的地方）
2. 逐字分析與練習：每個問題字說明是聲母、韻母還是聲調問題，給出正確讀法，附上有趣的練習方法（比如用順口溜、小故事等）
3. 用一段特別溫暖有趣的話鼓勵學生（可以用李白的故事來鼓勵）

如果所有字都正確，就寫一段超級開心的表揚，像李白一樣誇獎學生！
