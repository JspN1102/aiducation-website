...
export function mountFarewell(holder,{initialState,readOnly=false,playAudio,onState,onComplete,onResearch,reducedMotion=false}={}){
 ...
 root.innerHTML=`<div class="fs-control-panel"><header class="fs-header"><div><span class="fs-eyebrow">送別小劇場</span><h3></h3></div><span class="fs-progress"></span></header>
 <div class="fs-activity"><p class="fs-verse"></p><p class="fs-task"></p><div class="fs-rhythm" hidden><div class="fs-beats" aria-label="四個腳步">${rhythm.map((_,i)=>`<i data-fs-beat="${i}" aria-hidden="true">${i+1}</i>`).join('')}</div><div class="fs-feet"><button type="button" data-fs-foot="0"><span aria-hidden="true">‹</span>左腳</button><button type="button" data-fs-foot="1">右腳<span aria-hidden="true">›</span></button></div></div>
...
`;
 ...
 function next(){if(!available()||!passed()||stage===2)return;generation++;busy=false;stage++;selected=false;update();save();tell('');q('[data-fs-listen]').focus({preventScroll:true});}
