...
export async function createMountainViewer(holder,{signal,angle=0,onContextLost}={}){
 ...
 model.traverse(node=>{for(const material of [].concat(node.material||[]))for(const value of Object.values(material))if(value?.isTexture)value.anisotropy=Math.min(4,renderer.capabilities.getMaxAnisotropy());});
 ...
 return {
  ...
  capture(value=current){if(dead)return '';position(value);renderer.render(scene,camera);const data=canvas.toDataURL('image/webp',.82);position(current);renderer.render(scene,camera);return data;},
