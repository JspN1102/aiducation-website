import {readModel, createViewer, disposeObject} from './exploration.mjs?v=20260914e';

// Assessment viewer: no verse, vocabulary, answer caption or exploration hints.
export function mountChallengeModel(holder, {slug, modelURL, poster, label = '轉動觀察', controls = [], onListen} = {}) {
  const controller = new AbortController();
  let dead = false, busy = false, viewer = null, parsed = null, pending = null;
  holder.innerHTML = `<div class="challenge-model-stage"><img class="challenge-model-poster" alt="觀察畫面" decoding="async"><div class="challenge-model-canvas" hidden></div><span class="challenge-model-status" role="status"></span></div><div class="challenge-model-tools"><button type="button" data-model="load">${label}</button>${controls.map((c, i) => `<button type="button" data-model="preset" data-preset="${i}" hidden>${c.label}</button>`).join('')}${onListen ? '<button type="button" data-model="listen">聽聲音</button>' : ''}</div>`;
  ...
  async function load() {
    ...
    try {
      ...
      const response = await fetch(modelURL || `media/exploration/${slug}/model.glb`, {signal: request.signal, cache: 'no-cache'});
      ...
      viewer = createViewer({THREE, OrbitControls, gltf: parsed, holder: canvas, stage,
        content: {object: '觀察物件', initialView: modelURL ? [0, .12, 1] : slug === 'ti-xi-lin-bi' ? [1, .32, 0] : slug === 'gui-yuan-tian-ju' ? [0, 1.05, 1] : slug === 'zao-chun' ? [0,.14,1] : [3.3, 2.4, 5.8],
  ...
  holder.addEventListener('click', event => {
    ...
    if (button.dataset.model === 'listen') onListen?.();
