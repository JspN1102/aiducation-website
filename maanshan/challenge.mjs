...
export function mountChallenge(container, {poem, saved, onChange, onComplete, playAudio, stopAudio, recognize} = {}) {
  ...
  function intro() {
    ...
    startPage(`<section class="challenge-shell challenge-intro"><div class="challenge-intro-art"><img src="media/challenges/sound-market-v1.webp" alt="木製的聲音小鋪" width="960" height="800"><img class="challenge-intro-motif" src="media/poetry-motifs/${['goose','boat','mountain','moon','sprout','swallow'][poem.grade-1]}.svg" alt="" width="70" height="70"></div><div class="challenge-intro-copy"><p class="challenge-eyebrow">${esc(set.title)}</p><h2>帶着耳朵和小手，<br>來解開這首詩。</h2><p>聽兩次，寫兩個字，<br>最後動手完成一幅詩意。</p><div class="challenge-trip" aria-label="五個小挑戰"><span>聽</span><span>寫</span><span>聽</span><span>寫</span><span>玩</span></div><p class="challenge-kind-note">慢慢想，隨時可以再聽一次。</p><button class="challenge-primary" data-ch="start">${progress===5?'看看我的收穫':progress?`繼續第 ${Math.min(5,attempt.cursor+1)} 題`:'開始小挑戰'} <span aria-hidden="true">→</span></button>${progress ? '<button class="challenge-text-button" data-ch="restart">重新挑戰五題</button>' : ''}</div></section>`);
  }
