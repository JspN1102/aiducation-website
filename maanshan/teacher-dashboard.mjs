...
function studentPracticeBody(data){
 ...
 const perItem=summary.scope==='per-item',items=summary.items.slice(0,perItem?256:10),positions=new Set(items.map(item=>item.position));
 if(!perItem&&Number.isInteger(summary.total)&&summary.total>0&&summary.total<=10)for(let position=1;position<=summary.total;position++)if(!positions.has(position))items.push({position,type:null,status:'unanswered'});
 items.sort((a,b)=>(a.position??99)-(b.position??99));
 const hasGuided=items.some(item=>item.flow==='trace-dictation-v1');
 return `<section class="student-practice"><div class="student-practice-heading"><h3>練一練</h3><span>${esc(summary.title)}</span></div><p class="student-practice-progress">已完成 <strong>${shown(summary.completedN)}${!perItem&&summary.total?' / '+summary.total:''}</strong> 題<span>答對 <strong>${shown(summary.correctN)}</strong> 題</span></p><ol class="student-practice-list">${items.map((item,index)=>`<li><span class="practice-position">${perItem?index+1:item.position||index+1}</span><span>${esc(item.flow==='trace-dictation-v1'?'描紅與聽寫':types[item.type]||'待作答')}</span><strong class="practice-result ${item.status==='correct'||item.status==='completed'?'clear':item.status==='incorrect'?'needs-practice':''}">${esc(item.flow==='trace-dictation-v1'?(item.status==='completed'?'完成':'未完成'):status[item.status]||'未評分')}</strong></li>`).join('')}</ol><p class="chart-note">${perItem?'各題最近成績':'最近一輪'} · ${hasGuided?'描紅與聽寫、小遊戲只記完成情況':'小遊戲完成不計答對題數'}</p></section>`;
}
